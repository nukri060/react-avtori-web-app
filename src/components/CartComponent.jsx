...
export default function CartComponent({ name, price, image, quantity }) {
  const { updateQuantity, removeItem } = useContext(CartContext);
  const numericPrice = Number(price) || 0;

  const add = () => updateQuantity(name, quantity + 1);
  const subtract = () => updateQuantity(name, quantity - 1);
  ...
  return (
    <div className="flex items-center justify-between gap-4 bg-white border border-gray-200 rounded-xl shadow-sm p-4 hover:shadow-md transition-shadow duration-300">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <img className="w-16 h-16 object-cover rounded-md border" src={image} alt={name} />
      ...
      <div className="w-36 flex items-center justify-center gap-2">
        ...
        <button
          onClick={add}
          className="px-3 py-1 bg-blue-600 rounded-full text-white"
        >
          +
        </button>
      </div>
